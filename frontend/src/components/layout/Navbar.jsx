...
export default function Navbar() {
  ...
  const { appMode, setAppMode, checkMode } = useAppMode()

  const handleModeToggle = async () => {
    if (appMode === 'online') {
      setAppMode('offline')
    } else {
      // Immediately switch to online so nav links appear now
      setAppMode('online')
      // Verify in background — if backend is down, it will revert to offline
      checkMode()
    }
  }

  const links = [
    { to: '/selection', label: 'Home', Icon: HomeIcon, offline: true },
    { to: '/chatbot', label: 'Chatbot', Icon: SparklesIcon, offline: false },
    { to: '/planner', label: 'Planner', Icon: BookOpenIcon, offline: false },
    { to: '/progress', label: 'Progress', Icon: ChartBarIcon, offline: true },
    { to: '/teacher-validation', label: 'Review', Icon: ClipboardCheckIcon, offline: false },
  ]
      ...
      <nav className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <div className="hidden sm:flex items-center gap-1">
            {links.map(({ to, label, Icon, offline }) => {
              if (appMode === 'offline' && !offline) return null;
          ...
          <button
            id="app-mode-indicator"
            onClick={handleModeToggle}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold border transition-all duration-300 hover:scale-105 active:scale-95 shadow-sm
              ${appMode === 'online' ? 'bg-accent-teal/10 border-accent-teal/40 text-accent-teal hover:bg-accent-teal/20' : 'bg-accent-rose/10 border-accent-rose/40 text-accent-rose hover:bg-accent-rose/20'}
            `}
          >
          ...
          <div
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium
                        border transition-all duration-300 ${statusClass} ${isOnline && !isSyncing && pendingSyncCount === 0 ? 'hidden sm:flex opacity-0 pointer-events-none' : ''}`}
          >
            {!isOnline && <><WifiOffIcon className="w-3 h-3" />No Internet</>}
