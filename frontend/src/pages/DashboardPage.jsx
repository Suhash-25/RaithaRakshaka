...
import { LayoutDashboard, Microscope, MessageSquare, Cloud, BookOpen, TrendingUp, Heart, AlertTriangle, RefreshCw, ChevronRight } from 'lucide-react';
...
const CARD_LINKS = [
  ...
  { path: '/chat', label: 'AI Farm Advisor', desc: 'Multilingual chat assistant for agricultural tips', icon: MessageSquare, color: '#22c55e' },
  { path: '/weather', label: 'Weather Alerts', desc: 'Precipitation, wind speed, and AI advisories', icon: Cloud, color: '#60a5fa' },
  { path: '/schemes', label: 'Scheme Finder', desc: 'Government subsidies, loans, and benefits finder', icon: BookOpen, color: '#fbbf24' },
...
const FAST_DASHBOARD = {
  health_score: 87,
  location: 'Karnataka Farm Network',
  alert_count: 2,
  market_trend: 'rising',
  alerts: [
    { type: 'warning', message: 'Open Smart Map for exact field-level drought and disease risk.' },
    { type: 'info', message: 'Cached mandi signal is ready while live data refreshes.' },
  ],
  overview: 'Dashboard is ready instantly. Live farm intelligence refreshes in the background.',
};
...
function normalizeDashboard(d) {
  ...
  return {
    health_score: d?.health_score ?? d?.crop_health_score ?? FAST_DASHBOARD.health_score,
    location: d?.location || d?.live_context?.weather?.location || FAST_DASHBOARD.location,
    alert_count: d?.alert_count ?? d?.active_alerts ?? weatherAlerts.length ?? FAST_DASHBOARD.alert_count,
    market_trend: String(d?.market_trend || FAST_DASHBOARD.market_trend).toLowerCase(),
    alerts: weatherAlerts.length ? weatherAlerts : (d?.alerts || FAST_DASHBOARD.alerts),
    overview: d?.overview || d?.ai_tip || FAST_DASHBOARD.overview,
  };
}
    ...
    <div style={{ position: 'relative', width: 90, height: 90, margin: '0 auto' }}>
      <svg style={{ width: '100%', height: '100%', transform: 'rotate(-90deg)' }}>
        ...
        <motion.circle cx="45" cy="45" r="36" fill="transparent" stroke="#22c55e" strokeWidth="6"
          strokeDasharray={circ} initial={{ strokeDashoffset: circ }} animate={{ strokeDashoffset: offset }} transition={{ duration: 1 }} />
      </svg>
      <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ fontSize: '1.25rem', fontWeight: 800, color: '#22c55e' }}>{score}</span>
        <span style={{ fontSize: '0.55rem', color: '#4b7a58', fontWeight: 600, letterSpacing: '0.05em' }}>INDEX</span>
      </div>
    </div>
  );
}
...
export default function DashboardPage() {
  const [data, setData] = useState(FAST_DASHBOARD);
  ...
  const load = async () => {
    ...
    try {
      ...
      setData({
        ...
        market_trend: "rising",
        alerts: [
          { type: "warning", message: "Moderate rainfall forecast on May 24. Avoid chemical spraying." },
          { type: "info", message: "Tomato mandi price in Bangalore rose by ₹400/qtl." }
        ],
        overview: "Crop condition is stable. Ensure clean soil drainage to prevent fungal issues."
      });
  ...
  const getAlertColor = (t) => t === 'warning' ? '#fbbf24' : '#60a5fa';
    ...
    <div className="page-container">
      <div className="content-wrapper">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          <div>
            <div className="section-label" style={{ marginBottom: '0.4rem' }}>FARM DASHBOARD</div>
            <h1 style={{ fontFamily: 'Outfit', fontWeight: 800, fontSize: '2rem', color: '#f0fdf4' }}>Agri-Intelligence Overview</h1>
          </div>
          <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: 999, padding: '0.3rem 0.8rem' }}>
              <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#22c55e', animation: 'pulse-green 2s infinite' }} />
              <span style={{ color: '#4ade80', fontSize: '0.75rem', fontWeight: 600 }}>{data?.location || 'Pan India'}</span>
            </div>
            ...
            <div style={{ gridColumn: 'span 12', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.5rem' }}>
              ...
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} className="glass" style={{ borderRadius: 20, padding: '1.25rem', display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                ...
                <div>
                  ...
                  <p style={{ color: '#4b7a58', fontSize: '0.78rem', marginTop: '0.25rem' }}>Healthy leaf chlorophyll levels detected</p>
                </div>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="glass" style={{ borderRadius: 20, padding: '1.25rem' }}>
                ...
                <p style={{ color: '#86efac', fontSize: '0.75rem', marginTop: '0.25rem' }}>Immediate attention recommended</p>
              </motion.div>
              ...
              <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="glass" style={{ borderRadius: 20, padding: '1.25rem' }}>
                ...
                <div style={{ fontSize: '2rem', fontWeight: 900, color: '#4ade80' }}>
                  {data.market_trend === 'rising' ? 'Rising ↑' : 'Falling ↓'}
                </div>
                <p style={{ color: '#4b7a58', fontSize: '0.75rem', marginTop: '0.25rem' }}>Recommended: sell vegetables now</p>
              </motion.div>
            </div>
