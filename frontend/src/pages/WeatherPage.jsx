...
const FAST_WEATHER = {
  location: 'Bangalore',
  current: { temp: 29, humidity: 68, wind_speed: 8, condition: 'Clear', uv_index: 7, rainfall_mm: 0, feels_like: 31 },
  forecast: ['Today', 'Tue', 'Wed', 'Thu', 'Fri'].map((day, i) => ({ day, max: 31 + (i % 2), min: 22, condition: 'Clear', rain_prob: 25 + i * 5, humidity: 65 })),
  alerts: [{ type: 'info', icon: 'i', title: 'Fast Estimate', message: 'Showing instant estimate while live weather refreshes.', action: 'Use Smart Map for land-level scan' }],
  advisory: 'Fast estimate loaded instantly. Live weather refreshes in the background.',
};
...
export default function WeatherPage() {
  ...
  const [data, setData] = useState(FAST_WEATHER);
  ...
  const load = async (loc) => {
    ...
    try {
      ...
      setData((prev) => prev || FAST_WEATHER);
