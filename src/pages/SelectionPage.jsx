...
const BOARDS = [
  { id: 'state', label: 'State Board', icon: '🏛️', enabled: true, desc: 'Karnataka State Board syllabus (Class 1–12)' },
  { id: 'cbse', label: 'CBSE', icon: '🇮🇳', enabled: true, desc: 'Central Board of Secondary Education (All India)' },
  { id: 'icse', label: 'ICSE', icon: '📘', enabled: false, desc: 'Coming soon' },
]
...
export default function SelectionPage() {
  ...
  async function handleContinue() {
    ...
    if (selectedBoard === 'cbse') {
      // CBSE uses the new board-scoped learn route
      navigate(`/learn/cbse/${selectedClass.class_slug}/subjects`)
    } else {
  ...
  return (
    <div className="container-page animate-fade-in pb-32">
      ...
      <section className="mb-8 grid grid-cols-2 gap-4 md:grid-cols-4">
        <MetricTile 
          label="Total Classes" 
          value={catalog?.classes?.length || 12} 
        />
        <MetricTile 
          label="Total Subjects" 
          value={catalog?.classes?.reduce((acc, c) => acc + (c.subject_count || 0), 0) || 72} 
        />
        <MetricTile 
          label="Syllabus Pages" 
          value={formatCompactNumber(totalPages) || '5.2K'} 
        />
        <MetricTile 
              ...
              <button
                ...
                className={`rounded-xl border px-5 py-2.5 text-sm font-semibold transition-all ${
                  selectedBoard === board.id
                    ? 'border-primary-500 bg-primary-500 text-white shadow-[0_0_20px_rgba(108,99,255,0.4)]'
                    : board.enabled
                      ? 'border-surface-border bg-surface-card text-surface-muted hover:text-surface-text hover:border-primary-500 hover:bg-primary-500/10'
                      : 'cursor-not-allowed border-surface-border bg-surface-card/40 text-surface-muted/40'
                }`}
              >
                {board.label}
                {!board.enabled && <span className="ml-2 text-xs opacity-50">Soon</span>}
              </button>
            ))}
          </div>
        </div>
