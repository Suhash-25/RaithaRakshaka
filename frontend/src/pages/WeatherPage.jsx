...
export default function WeatherPage() {
  ...
  const load = async (loc) => {
    ...
    try {
      ...
      setError('Live weather service is unavailable. Please confirm the backend is running.');
        ...
        {loading && !data ? (
          ...
          <>
            <div className="live-pill" style={{ marginBottom: '1rem', width: 'fit-content' }}>
              ...
              <span>{data.source || 'Live weather intelligence'} {data.updated_at ? `- ${new Date(data.updated_at).toLocaleTimeString()}` : ''}</span>
            </div>
                  ...
                  {data.forecast.map((f, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.6rem 0.75rem', background: 'rgba(255,255,255,0.03)', borderRadius: 10 }}>
                      ...
                      <div style={{ textAlign: 'right' }}>
                        ...
                        <div style={{ color: '#60a5fa', fontSize: '0.7rem' }}>Rain {f.rain_prob}%</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <motion.div className="glass" style={{ borderRadius: 20, padding: '1.5rem', marginBottom: '1.5rem' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(250px,1fr))', gap: '0.75rem' }}>
                {(data.alerts?.length ? data.alerts : [{ type: 'info', title: 'No active alerts', message: 'Live service returned normal conditions.', action: 'Continue field monitoring' }]).map((a, i) => (
